...
function textFont(option) {
    if (option.value == 'time') {
        text.style.cssText = 'font-family : Times New Roman, Times, serif';
    } else if (option.value == 'arial') {
        text.style.cssText = 'font-family : Arial';
    } else if (option.value == 'cursive') {
        text.style.cssText = 'font-family : cursive';
    } else if (option.value == 'sans') {
        text.style.cssText = 'font-family : sans-serif';
...
button.addEventListener('click', () => {
    ...
    if (file.files.length == 0) { //Flags an error when no file is uploading
        ...
        if (ext == 'txt') {//checks if file is a .txt file
            alert('Press space bar inside the text box after submitting file.');
        } else {
